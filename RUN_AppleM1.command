...
for ziplog in "$MY_PATH"/input/*.zip; do
    ...
    docker run --rm --platform linux/amd64 -v "$ziplog":/app/logs.zip -v "$MY_PATH"/output:/app/Output -v "$MY_PATH"/LogSharkConfig.json:/app/Config/LogSharkConfig.json logshark:latest logs.zip --plugins "ALL" -p |& tee -a "$MY_PATH"/logs/runresults.log
